...
#здесь идёт рекурсивная реализация с мемоизацией
def F_recursive(n, memo={}):
    ...
    if n not in memo:
        ...
        else:  #Если n нечетное, как раз else отвечает
...
def F_iterative(n):
    ...
    for i in range(2, n + 1):
        if i % 2 == 0:  # Если i четное
            ...
            factorial_2i = calculate_factorial(2 * i)
            current = ((i - 2) / factorial_2i) - prev
        prev = current
    return prev

#функция для ручного вычисления факториала, а не через функцию
